...
func _physics_process(delta):
	for mover in movers.get_children():
		...
		if mover.is_in_liquid:
			...
			var lift = process_lift(mover.velocity, mover.rect_height, get_node("Liquid"))
			mover.apply_force(lift)
...
func process_lift(obj_velocity, obj_surface, liquid):
	var speed = obj_velocity.length()
	var area = obj_surface *0.1
	var lift_magnitude = liquid.coefficient * speed * speed * area
	var lift = obj_velocity * -1
	lift = lift.normalized()
	lift *= lift_magnitude
	#rotates 90degrees, always lifts up
	lift = Vector2(lift.y, -abs(-lift.x))
	return lift
