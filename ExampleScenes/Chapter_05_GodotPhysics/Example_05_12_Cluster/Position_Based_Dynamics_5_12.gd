#comes from John Meadow's 2019 Godot Con presentation: github.com/JohnMeadow1

extends Node2D

class_name Position_Based_Dynamics_5_12

export var is_stiff_body:bool = false
export var is_tearable_body:bool = false

export var compress_stiffness:= 0.1
export var stretch_stiffness := 0.1

export var external_force := Vector2( 0.0, 0.0 )
export var gravity := Vector2( 0,  0)
export var rest_length = 100

export var damping := 1

var correction:= Vector2.ZERO

func apply_external_forces( node, delta ):
...
func add_vertex( new_position:Vector2 = Vector2( 300.0, 300.0 ), is_static:bool = false, new_mass:float = 1.0 ) -> void :
	var new_point_mass := PointMass_5_12.new() as PointMass_5_12
...
func add_spring( node_index_1, node_index_2, new_compress, new_stretch  )->void:
	var link := Distance_Constraint_5_12.new()
	(link as Distance_Constraint_5_12).initialize( $verticles.get_child(node_index_1), $verticles.get_child(node_index_2), rest_length)
