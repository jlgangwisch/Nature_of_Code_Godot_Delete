...
func _input(event):
	...
	if event is InputEventMouseButton:
		if event.pressed:
			...
			for node in get_node("Creature_5_14").get_node("verticles").get_children():
				if event.position.distance_to( node.global_transform.origin ) < selectionDistance:
					selection = node
					break
			for node in get_node("MultipleClusters_5_15").get_node("verticles").get_children():
				if event.position.distance_to( node.global_transform.origin ) < selectionDistance:
					selection = node
					break
